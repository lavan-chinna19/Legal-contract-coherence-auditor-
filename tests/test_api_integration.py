"""
tests/test_api_integration.py — Local API Integration Test (Prompts 15 & 16)
"""
import os
import time
import uuid
...
# Ensure a dynamic test key is configured in environment (no hardcoded secret)
TEST_KEY = os.environ.get("API_KEY") or str(uuid.uuid4())
...
def test_health_and_readiness():
    """GATE 1: API starts locally and /health succeeds without authentication."""
...
def test_end_to_end_analysis_flow():
    """
    GATE 2 & 3: End-to-end integration and programmatic comparison with authentication.
    ...
    # 4. Get Results (Authenticated)
    response = client.get(f"/v1/jobs/{job_id}/results", headers=AUTH_HEADERS)
